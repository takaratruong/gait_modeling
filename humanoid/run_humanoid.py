...
if __name__ == '__main__':
    ...
    train_env = make_vec_env(lambda:HumanoidEnv(args=args), n_envs=1, seed=0, vec_env_cls=DummyVecEnv)
    #eval_env = make_vec_env(lambda:HumanoidEnv(args=args), n_envs=1, vec_env_cls=DummyVecEnv)
    ...
    model = PPO("MlpPolicy", train_env, batch_size=(args.num_envs*args.num_steps)//4, use_sde=False, n_steps= args.num_steps, n_epochs=args.num_epochs, gamma=.99, gae_lambda=.95,
                target_kl=.01, verbose=True,  clip_range=.1, ent_coef=.000585045, vf_coef=0.871923, max_grad_norm=10, learning_rate=1e-4, policy_kwargs=policy_kwargs)
